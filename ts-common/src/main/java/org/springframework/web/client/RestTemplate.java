...
public class RestTemplate extends InterceptingHttpAccessor implements RestOperations {
    ...
    public <T> ResponseEntity<T> exchange(URI url, HttpMethod method, @Nullable HttpEntity<?> requestEntity,
        ...
        RequestCallback requestCallback = httpEntityCallback(requestEntity, responseType);
        ResponseExtractor<ResponseEntity<T>> responseExtractor = responseEntityExtractor(responseType);
        ResponseEntity<T> re = nonNull(execute(url, method, requestCallback, responseExtractor));
        logTime.springExitEnd.add(System.nanoTime());
        return nonNull(re);
    }
    ...
    public <T> ResponseEntity<T> exchange(RequestEntity<?> requestEntity, Class<T> responseType)
        ...
        RequestCallback requestCallback = httpEntityCallback(requestEntity, responseType);
        ResponseExtractor<ResponseEntity<T>> responseExtractor = responseEntityExtractor(responseType);
        ResponseEntity<T> re = nonNull(doExecute(requestEntity.getUrl(), requestEntity.getMethod(), requestCallback, responseExtractor));
        logTime.springExitEnd.add(System.nanoTime());
        return nonNull(re);
    }
